class DataStore {
  ...
  normalizePlayerStat(stat, playerById = new Map()) {
    ...
    return {
      ...
      roleSource: DataStore.ROLES.has(source.rolePlayed) ? (['manual', 'ocr', 'inferred', 'legacy', 'roster'].includes(source.roleSource) ? source.roleSource : 'legacy') : 'unknown',
  ...
  normalizeMatch(match, playerById = new Map(), now = new Date().toISOString()) {
    ...
    return {
      ...
      sourceBattleId: typeof source.sourceBattleId === 'string' && /^\d{10,24}$/.test(source.sourceBattleId) ? source.sourceBattleId : null,
      verificationStatus,
      ocrVerified,
      needsReview,
