...
export function parseOcrProviderResponse(payload, { batchCount = 0 } = {}) {
  ...
  if (batchCount) {
    if (!Array.isArray(parsed?.files) || parsed.files.length !== batchCount) throw unusable('invalid_shape');
    return parsed;
  }
  ...
  for (const model of modelList) {
    ...
    try {
      ...
      const work = async () => {
        ...
        const parsed = parseOcrProviderResponse(payload, { batchCount });
