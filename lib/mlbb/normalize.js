...
export function normalizeHeroCatalog(payload, options = {}) {
  ...
  const heroes = records.map(record => {
    ...
    return {
      ...
      name,
      aliases: [],
      roles: unique([...(Array.isArray(heroData.sortlabel) ? heroData.sortlabel : []), ...nestedLabels(heroData.sortid, 'sort_title')]),
      lanes: unique([...(Array.isArray(heroData.roadsortlabel) ? heroData.roadsortlabel : []), ...nestedLabels(heroData.roadsort, 'road_sort_title')]),
      specialties: [],
      story: '',
      images: {
...
export function normalizeCounterPayload(payload, options = {}) {
  const matchupRate = (value, min = 0) => value !== null && value !== undefined && value !== '' && Number.isFinite(Number(value)) && Number(value) >= min && Number(value) <= 1 ? Number(value) : null;
  ...
  const normalizeRows = rows => (Array.isArray(rows) ? rows : []).map(row => ({
    ...
    winRate: matchupRate(row?.hero_win_rate),
    pickRate: matchupRate(row?.hero_appearance_rate),
    deltaWinRate: matchupRate(row?.increase_win_rate, -1)
