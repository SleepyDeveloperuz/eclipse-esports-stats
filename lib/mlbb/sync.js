...
import { parsePatchArticle } from './patch-parser.js';
...
async function syncRank(context) {
  ...
  const tiers = buildEclipseTier(rows);
  ...
  const comparison = buildTierComparison(previous, tiers, { rank, days, now });
  ...
  const historyValue = await store.getJSON(MLBB_KEYS.rankHistory);
  const date = now.slice(0, 10);
  const history = Array.isArray(historyValue?.data) ? historyValue.data.filter(item => !(item?.date === date && item.rank === rank && item.days === days)) : [];
  history.push({
    date,
    rank,
    days,
    updatedAt: now,
    methodologyVersion: tierMethodology().version,
    tiers: tiers.map(row => ({ heroId: row.heroId, tier: row.tier, eclipseRank: row.eclipseRank })),
    heroes: rows.map(row => ({
      heroId: row.heroId,
      winRate: row.winRate,
  ...
  await store.setJSON(MLBB_KEYS.rankHistory, envelope(history.slice(-360), now, value.source));
  return value;
}

export function buildTierComparison(previous, tiers, { rank, days, now }) {
  ...
  return { previousUpdatedAt: previous.updatedAt, methodologyVersion: tierMethodology().version, changes: tiers.map(row => {
    ...
    return { heroId: row.heroId, previousTier: before?.tier || null, tier: row.tier,
      rankDelta: before ? Number(before.eclipseRank) - Number(row.eclipseRank) : null, isNew: !before };
  }) };
}
...
async function syncPatches(context, catalogValue) {
  ...
  const candidates = feed
    .filter((item, index) => /patch notes/i.test(item.title) && (index === 0 || item.id === latestRelease?.id || !previousById.get(item.id)?.parsedAt))
