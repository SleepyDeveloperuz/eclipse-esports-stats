...
export const PATCH_PARSER_VERSION = 'patch-sections-2';
...
export function parsePatchArticle(article, catalog = [], options = {}) {
  ...
  lines.forEach((line, index) => {
    ...
    const section = [];
    for (let next = index + 1; next < lines.length && section.length < 20; next++) {
      const heading = lines[next].match(/^\[([^\]]+)\]\s*\([↑↓~]\)$/);
      if ((heading && heroNames.has(normalizeHeroKey(heading[1]))) || /^\d+\.\s/.test(lines[next])) break;
      section.push(lines[next]);
    }
    let skill = '';
    const changes = section.flatMap(text => {
      const heading = text.match(/^\[([^\]]+)\](?:\s*\([↑↓~]\))?$/);
      if (heading) { skill = heading[1]; return []; }
      return [{ skill, text: text.slice(0, 1000) }];
    });
    adjustments.push({
      heroId: hero.id,
      heroName: hero.name,
      change: changeType(match[2]),
      summary: firstNarrative(lines, index).slice(0, 320), changes
    });
  });
