...
function normalisePlayerStat(stat) {
  ...
  return {
    ...
    roleSource: ROLES.has(stat?.rolePlayed) ? (['manual', 'ocr', 'inferred', 'legacy', 'roster'].includes(stat?.roleSource) ? stat.roleSource : 'legacy') : 'unknown',
...
function normaliseMatch(match) {
  ...
  return {
    ...
    sourceBattleId: typeof match?.sourceBattleId === 'string' && /^\d{10,24}$/.test(match.sourceBattleId) ? match.sourceBattleId : null,
    verificationStatus,
    ocrVerified,
    needsReview,
